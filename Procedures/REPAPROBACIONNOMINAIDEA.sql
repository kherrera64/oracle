         ...
         UNION SELECT D.CODPERS, E.NOMBRE1||' '||E.NOMBRE2 || ' '|| E.APELLIDO1 
                      ...
                      NVL((SELECT DESCRIPCION
                          ...
                          (SELECT DESCRIPCION
                            ...
                            nvl((select INICIO_PAGO 
                              from DBAFISICC.nodocxhorariotb
                              where horario = d.horario
                              and codpers = d.codpers
                              and codpuesto = d.codpuesto
                              and resolucion = d.solicitud),
                              (select INICIO_PAGO 
          ...
          WHERE D.CODPERS = E.CODPERS) A, DBAFISICC.sosoltramitetb b
      WHERE B.SOLICITUD = A.SOLICITUD
      AND B.STATUS = 'A'
      AND (A.TIPO = PTIPO OR PTIPO IS NULL)
